...
// แกลเลอรี "ตัวอย่างผลงาน" — ใช้ร่วมกันทั้งหน้า Course Detail และ Ebook
// ระบบมีทั้งรูปและคลิป YouTube คละแนวนอน/แนวตั้ง — ถ้าปล่อยไหลรวมกันจะเกิดช่องโหว่
// ริมการ์ดไม่เท่ากัน เลยจัดกลุ่ม: แนวนอนเรียงกริดของตัวเองก่อน แล้วแนวตั้งต่อเป็น
// กริดแยกอีกแถว → เต็มแถวเรียบทุกส่วนผสม (ลำดับที่แอดมินจัดยังคงอยู่ภายในกลุ่ม)
const SamplesGallery = ({ samples }: { samples: SampleItem[] }) => {
  const landscape = samples.filter((s) => s.orientation !== 'portrait');
  const portrait = samples.filter((s) => s.orientation === 'portrait');

  const renderItem = (s: SampleItem, i: number) => (
