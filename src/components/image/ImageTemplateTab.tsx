...
import { ArrowRight, ChevronDown, Search, Sparkles, Star, Plus, Loader2 } from 'lucide-react';
...
type ModelKey = 'gpt-image-2' | 'grok-imagine' | 'nano-banana-2' | 'nano-banana-pro';

// Templates now come from DB via /api/image-template/list (admin-managed in /admin/image-templates)
...
// Models that appear in the user-facing model filter dropdown.
// Other ModelKey values still resolve via MODEL_LABELS for any legacy template that uses them.
const VISIBLE_MODELS: ModelKey[] = ['gpt-image-2', 'nano-banana-2'];

const FAVORITES_KEY = 'imageTemplateFavorites';
...
function ImageTemplateCard({
  ...
  isFavorite: boolean;
  isNew: boolean;
  onUse: (template: ImageTemplate) => void;
    ...
    >
      <div className="relative">
        ...
        <div className="aspect-[9/16] bg-zinc-950 relative flex items-center justify-center overflow-hidden rounded-t-[7px]">
          <img
            src={(template.thumbnail_url || '').replace(/([?&])dl=1(\b|$)/, '$1raw=1$2')}
            alt={displayName}
...
export function ImageTemplateTab() {
  ...
  const useTemplate = (template: ImageTemplate) => {
    navigate(`/app/images-template/${encodeURIComponent(template.slug)}`);
  };

  const [allTemplates, setAllTemplates] = useState<ImageTemplate[]>([]);
  const [loadingTemplates, setLoadingTemplates] = useState(true);

  useEffect(() => {
    let cancelled = false;
    setLoadingTemplates(true);
    api.getImageTemplatesPublic()
      .then((resp) => {
        if (cancelled) return;
        setAllTemplates(Array.isArray(resp?.items) ? resp.items : []);
      })
      .catch((err) => {
        if (cancelled) return;
        console.warn('Failed to load image templates:', err?.message);
        setAllTemplates([]);
      })
  ...
  const visibleTemplates = allTemplates
    ...
    .filter((template) => {
      ...
      return [
        ...
        template.description_th,
        MODEL_LABELS[template.model],
      ].filter(Boolean).some((value) => (value as string).toLowerCase().includes(q));
    })
    .sort((a, b) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
        ...
        <div className="relative" ref={modelMenuRef}>
          ...
          {showModelMenu && (
            <div className="absolute top-full left-0 mt-1.5 min-w-[160px] rounded-xl bg-background border border-border shadow-xl py-1 z-50">
              {VISIBLE_MODELS.map((key) => [key, MODEL_LABELS[key]] as [ModelKey, string]).map(([key, label]) => (
                <button
                  key={key}
                  onClick={() => {
      ...
      {filter === 'custom' ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-zinc-600">
          <Loader2 className="h-8 w-8 mb-3 animate-spin text-[#FFB300]" />
          <p className="text-sm">{l('กำลังโหลด Template...', 'Loading templates...')}</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          {visibleTemplates.map((template, idx) => (
            <ImageTemplateCard
              key={template.slug}
              template={template}
              displayName={(language === 'th' ? template.name_th : template.name) || template.name}
              description={(language === 'th' ? template.description_th : template.description) || ''}
        ...
        <div className="flex flex-col items-center justify-center py-16 text-zinc-600">
          ...
          <p className="text-sm font-medium mb-2">
            {filter === 'favorite'
              ...
              : l('ไม่พบ Template', 'No templates found')}
          </p>
          <button
            type="button"
            onClick={() => setFilter('all')}
...
export default ImageTemplateTab;
