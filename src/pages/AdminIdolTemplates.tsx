...
import type { IdolTemplateAdmin, TemplateVariable, FieldConfig } from '@/types/idolTemplate';
...
const emptyTemplate: Partial<IdolTemplateAdmin> = {
  slug: '', name: '', description: '', thumbnail_url: '', preview_video_url: '',
  input_mode: 'single', input_label: '', input_placeholder: '',
  system_prompt: '', display_order: 0, is_active: true, yearly_only: false, gender: null,
...
const AdminIdolTemplates = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const [templates, setTemplates] = useState<IdolTemplateAdmin[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<Partial<IdolTemplateAdmin> | null>(null);
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true);
      const data = await api.getIdolTemplatesAdmin();
      setTemplates(data);
    } catch (err: any) {
  ...
  const handleEdit = (t: IdolTemplateAdmin) => {
    setEditing({ ...t });
    setIsNew(false);
  ...
  const handleSave = async () => {
    if (!editing) return;
    if (!editing.name || !(editing.image_prompt_template || editing.system_prompt)) {
      toast.error('ชื่อ และ Image Prompt จำเป็นต้องกรอก');
      return;
    }
  ...
  const handleDelete = async (slug: string) => {
    ...
    try {
      await api.deleteIdolTemplate(slug);
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/app/idol-templates')}>
              <ArrowLeft className="w-4 h-4 mr-1" /> กลับ
            </Button>
            <h1 className="text-2xl font-bold">Idol Template Manager</h1>
          </div>
          {!editing && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input className="bg-gray-800 border-gray-700 text-white mt-1"
                      ...
                      placeholder="/idol-templates/my-thumbnail.jpg"
                    />
                  </div>
                </div>
            ...
            <Card className="bg-gray-900 border-gray-800">
              <CardContent className="p-5 space-y-4">
                ...
                <div className="border-t border-gray-800 pt-3 space-y-2">
                  <p className="text-xs text-gray-400 font-medium">Idol Template: User อัพรูป Idol + รูปชุด → AI สร้าง prompt → สร้างภาพ → สร้าง VDO (1 Task = 1 VDO)</p>
                </div>
              </CardContent>
            </Card>

            {/* ── Section 4: Image Prompt Template ── */}
            <Card className="bg-gray-900 border-gray-800">
              <CardContent className="p-5 space-y-3">
                <h3 className="text-sm font-bold text-yellow-400 uppercase tracking-wide">Image Prompt *</h3>
                <Textarea
                  className="bg-gray-800 border-gray-700 text-white font-mono text-sm"
                  rows={10}
                  value={editing.image_prompt_template || ''}
                  onChange={e => setEditing(prev => ({ ...prev, image_prompt_template: e.target.value }))}
                  placeholder="Prompt สำหรับสร้างรูปภาพ เช่น amateur smartphone photo, realistic casual snapshot..."
                />
                <p className="text-xs text-gray-500">
                  Prompt นี้จะถูกส่งไป Nanobanana 2 เพื่อสร้างรูปภาพโดยตรง
                </p>
              </CardContent>
            </Card>

            {/* ── Section 5: Video Prompt Template ── */}
            <Card className="bg-gray-900 border-gray-800">
              <CardContent className="p-5 space-y-3">
                <h3 className="text-sm font-bold text-yellow-400 uppercase tracking-wide">Video Prompt *</h3>
                <Textarea
                  className="bg-gray-800 border-gray-700 text-white font-mono text-sm"
                  rows={6}
                  value={editing.video_prompt_template || ''}
                  onChange={e => setEditing(prev => ({ ...prev, video_prompt_template: e.target.value }))}
                  placeholder="Prompt สำหรับสร้าง VDO เช่น slow stretching motion, eye contact with camera..."
                />
                <p className="text-xs text-gray-500">
                  Prompt นี้จะถูกส่งไป Grok image-to-video เพื่อสร้างวิดีโอจากรูปที่ได้
                </p>
              </CardContent>
            </Card>
...
export default AdminIdolTemplates;
