...
/* ================================================================== */
/*  Coupon Codes — bulk-create / list / toggle (admin)                  */
/* ================================================================== */

/**
 * GET /api/admin/coupons
 * Query: status=all|active|redeemed|inactive, q, limit, offset
 *
 * All admins can view + manage existing codes (edit notes, toggle active).
 * Only *Super* admin can CREATE new codes — see the POST handler below.
 */
router.get('/coupons', authenticate, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const status = (req.query.status as any) || 'all';
    const q = (req.query.q as string) || '';
    const limit = Math.min(Number(req.query.limit) || 50, 200);
    const offset = Math.max(Number(req.query.offset) || 0, 0);
    const result = await couponService.listCoupons({ status, q, limit, offset });
    res.json({ coupons: result.rows, total: result.total, limit, offset });
  } catch (err: any) {
 ...
 * Body: { days, count, notes? }
 * Super admin only — bulk creates `count` codes worth `days` days each.
 */
router.post(
  '/coupons',
  authenticate,
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      // max_uses: undefined → service default (1 = single-use); null → unlimited; N → capped.
      let maxUses: number | null | undefined = undefined;
      if (req.body?.max_uses !== undefined) {
        const raw = req.body.max_uses;
        if (raw === null) {
          maxUses = null;
        } else {
          const n = Number(raw);
      ...
      if (!Number.isInteger(count) || count <= 0 || count > couponService.MAX_BULK_COUNT) {
        return res.status(400).json({ error: `count must be 1..${couponService.MAX_BULK_COUNT}` });
      }
      const adminId = req.userId!;
      const created = await couponService.bulkCreate(days, count, adminId, notes, maxUses);
      res.json({ success: true, created });
    } catch (err: any) {
      console.error('[admin/coupons] create failed:', err);
      res.status(500).json({ error: err?.message || 'Failed to create coupons' });
    }
  }
);
 ...
 * Body: { is_active?, notes? } — partial update
 *
 * Any admin can pause/resume codes or update notes. Only Create requires
 * Super admin (see POST above). Coupon `code` + `days` are immutable.
 */
router.patch(
  '/coupons/:id',
  authenticate,
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isInteger(id)) return res.status(400).json({ error: 'Invalid id' });
      const patch: { is_active?: boolean; notes?: string | null; max_uses?: number | null } = {
      ...
      // max_uses: null = unlimited; N = capped. undefined leaves it alone.
      if (req.body?.max_uses !== undefined) {
        const raw = req.body.max_uses;
        if (raw === null) {
          patch.max_uses = null;
      ...
      const updated = await couponService.setCouponState(id, patch);
      if (!updated) return res.status(404).json({ error: 'coupon not found' });
      res.json({ success: true, coupon: updated });
    } catch (err: any) {
      console.error('[admin/coupons] patch failed:', err);
 ...
 * Hard-delete an unredeemed coupon. Refuses to delete redeemed coupons —
 * those carry audit info (who/when) and should be deactivated instead.
 * Any admin can delete (consistent with the PATCH gating — only Create
 * is super-admin gated).
 */
router.delete(
  '/coupons/:id',
  authenticate,
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (result === null) return res.status(404).json({ error: 'coupon not found' });
      if (result === 'already_redeemed') {
        return res.status(409).json({
          error: 'already_redeemed',
          message: 'Cannot delete a redeemed coupon. Deactivate it instead.',
        });
      }
